...
class AccountAssetSell(models.TransientModel):
    ...
    def action_transfer(self):
        ...
        # self.create_account_move_receiver()
    ...
    def get_account_move_vals(self):
        journal_id = self.asset_id.company_id.x_journal_internal_id
        debit_account_id = self.asset_id.company_id.x_internal_payable_account_id
    ...
    def create_account_move_receiver(self):
        receive_company = self.x_receive_company_id
        journal_id = receive_company.x_journal_internal_id
        debit_account_id = receive_company.x_internal_payable_account_id
        if (not journal_id or journal_id is None) or (not debit_account_id or debit_account_id is None):
            raise ValidationError(
                _('Please config Transfer Journal and Payable Account in Currency %s') % self.asset_id.currency_id.name)
        today = datetime.today().date()
        ...
        self.env['account.move'].sudo().create(account_move_argvs)
