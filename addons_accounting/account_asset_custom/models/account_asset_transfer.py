...
class AssetTransfer(models.TransientModel):
    ...
    x_receive_company_id = fields.Many2one(
        'res.company', 'Receive Company', domain=_get_domain_receive_company)

    # Confirm Transfer account asset
    def action_transfer(self):
        # checking depreciation_move
        number_move_draft = len(
            self.asset_id.depreciation_move_ids.filtered(lambda d: d.state == 'draft'))
        # checking transfer to same company
        if self.x_receive_company_id == self.env.company:
            raise ValidationError(
                _('Cannot transfer asset to same company. Please select other company!'))

        # Validate state
        if self.asset_id.state not in ['open', 'paused', 'close']:
            raise ValidationError(
                _('Cannot transfer asset not in state OPEN, PAUSED, CLOSE!'))
        ...
        account_move_id = self.env['account.move'].sudo().create(
            account_move_argvs)
        _logger.info(
            f"account_asset_custom: NEW Account move: {account_move_id.id}")
        ...
        _logger.info(
            f"account_asset_custom: NEW Asset: {receive_company_asset.id}")
    ...
    def get_account_move_vals(self):
        ...
        debit_account_id = self.asset_id.currency_id.receivable_account_id
        ...
        # return {
        #     'ref': _(
        #         'Transfer') + f' {self.asset_id.name} - {self.asset_id.x_code} - {self.asset_id.company_id.name} - {self.x_receive_company_id.name}',
        ...
        # Sửa lại theo ví dụ trong tài liệu
        # Nợ 1368: Giá trị còn lại chuyển đi(16.000.000)
        # Nợ 2141: Giá trị đã khấu hao lũy kế(14.000.000)
        # Có 2111: Nguyên giá tài sản(30.000.000)

        print("XXXXXXXXXXXXTTTTTTTTTTTTTTTTTTTTTTTTT")
        return {
            'ref': _(
                'Transfer') + f' {self.asset_id.name} - {self.asset_id.x_code} - {self.asset_id.company_id.name} - {self.x_receive_company_id.name}',
            'journal_id': journal_id.id,
            ...
            'line_ids': [
                (0, 0, self.get_account_move_line_vals(account_id=debit_account_id.id,
                                                       ...
                                                       name=_('Receivable Value for Asset: ') + self.asset_id.name + ' ' + self.asset_id.x_code)),
                (0, 0, self.get_account_move_line_vals(account_id=self.asset_id.account_depreciation_id.id,
                                                       debit=asset_depreciated_value,
                                                       name=_('Depreciated Value for Asset: ') + self.asset_id.name + ' ' + self.asset_id.x_code)),
                (0, 0, self.get_account_move_line_vals(account_id=self.asset_id.account_asset_id.id,
                                                       credit=self.asset_id.value_residual + asset_depreciated_value,
                                                       name=_('Value Residual for Asset: ') + self.asset_id.name + ' ' + self.asset_id.x_code))
            ]
        }
    ...
    def get_asset_depreciated_value(self):
        last_posted_move_ids = self.asset_id.depreciation_move_ids.filtered(
