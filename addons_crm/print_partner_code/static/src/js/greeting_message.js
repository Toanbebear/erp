odoo.define('patient.greeting_message', function (require) {
"use strict";
...
var GreetingMessage = AbstractAction.extend({
    contentTemplate: 'PtAttendanceGreetingMessage',
...
//                if(has_group) {
//                    self.next_action = 'patient.pt_attendance_action_kiosk_mode';
//                } else {
//                    self.next_action = 'patient.pt_attendance_action_my_attendances';
//                }
//            });
//            return;
//        }

        this.next_action = action.next_action || 'patient.pt_attendance_action_my_attendances';
        // no listening to barcode scans if we aren't coming from the kiosk mode (and thus not going back to it with next_action)
        if (this.next_action != 'patient.pt_attendance_action_kiosk_mode' && this.next_action.tag != 'pt_attendance_kiosk_mode') {
            this.activeBarcode = false;
        }
        ...
        if(this.attendance.check_in != "" || this.attendance.check_in != null || this.attendance.check_in)
        {
            this.attendance.check_in = this.attendance.check_in && moment.utc(this.attendance.check_in).local();

        }
        if (this.attendance.check_out != "" || this.attendance.check_out != null || this.attendance.check_out){
            ...
            this._rpc({
                ...
                .then(function (result) {
                    if (result.action) {
                        ...
                        setTimeout( function() { self.do_action(self.next_action, {clear_breadcrumbs: true}); }, 500);
                    }
                }, function () {
                    setTimeout( function() { self.do_action(self.next_action, {clear_breadcrumbs: true}); }, 500);
                });
...
//core.action_registry.add('pt_attendance_greeting_message', GreetingMessage);
//
