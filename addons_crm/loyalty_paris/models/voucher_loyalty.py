...
class VoucherLoyalty(models.Model):
    _name = 'voucher.loyalty'
    _description = 'Cấu hình Voucher'

    name = fields.Char('Tên voucher', required=True)
    type = fields.Selection([('prd', 'Sản phẩm'), ('service', 'Dịch vụ')], string='Loại voucher', required=True, default='prd')
    method = fields.Selection([('gift','Tặng'), ('discount','Giảm giá')], string='Hình thức ưu đãi', required=True, default='gift')
    ...
    stage = fields.Selection([('allow', 'Đang sử dụng'), ('not_allow', 'Không được sử dụng')], string='Trạng thái', default='allow', readonly=True)
    ...
    def create(self, vals):
        res = super(VoucherLoyalty, self).create(vals)
        if res and res.is_birth_day == False:
    ...
    def write(self, vals):
        res = super(VoucherLoyalty, self).create(vals)
        if res:
            reward = self.env['crm.loyalty.line.reward'].sudo().search([('voucher_loyalty_id','=',self.id)])
            reward.sudo().write({
                    'name': self.name,
    ...
    def button_cancel(self):
        if self.stage == 'allow':
            ...
            where voucher_loyalty_id = %s and stage = 'allow'
            """
