...
class CollaboratorReportPayment(models.TransientModel):
    _name = 'crm.collaborator.payment.report'
    ...
    def _get_data_report(self):
        ...
        if self.type_report == 'type_2':
            if self.collaborators_id:
                ret_data = []
                domain = [('ctv_payment_date', '>=', self.start_date), ('ctv_payment_date', '<=', self.end_date), ('company_id', '=', self.company_id.id),
                          ('collaborators_id', '=', self.collaborators_id.id), ('state', '=', 'posted')]
    ...
    def create_report(self):
        ...
        # in du lieu
        report_brand_overview_attachment = self.env['ir.attachment'].browse(self.env.ref('crm_collaborators.bao_cao_hoa_hong_cong_tac_vien_attachment').id)
