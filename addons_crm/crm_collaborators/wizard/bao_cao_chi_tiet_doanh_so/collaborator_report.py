...
class CtvReport(models.TransientModel):
    ...
    def _get_data_report(self):
        ...
        if self.type_report == 'type_2':
            if self.collaborators_id:
                ret_data = []
                domain = [('service_date', '>=', self.start_date), ('service_date', '<=', self.end_date), ('company_id', '=', self.company_id.id), ('collaborators_id', '=', self.collaborators_id.id)]
                ...
                for record in detail:
                    ret_data.append({
                        ...
                        "ma_ctv": record.collaborators_id.code_collaborators,
                        "ten_ctv": record.collaborators_id.name,
                        ...
                        "ngan_hang": record.collaborators_id.bank if record.collaborators_id.bank else None,
                        "so_tai_khoan": record.collaborators_id.card_number if record.collaborators_id.card_number else None,
                        "ten_chu_tai_khoan": record.collaborators_id.user_bank if record.collaborators_id.user_bank else None,
                        "ghi_chu": None,
                    })
                    stt += 1
            ...
            ret_data = []
            domain = [('service_date', '>=', self.start_date), ('service_date', '<=', self.end_date), ('company_id', '=', self.company_id.id)]
            detail = self.env['crm.detail.sale'].search(domain)
            ...
            for record in detail:
                ret_data.append({
                    ...
                    "ngay_thuc_hien": record.walkin_id.service_date_start.strftime('%d/%m/%Y') if record.walkin_id.service_date_start else None,
        ...
        #
        #     # ctv = self.env['crm.collaborators'].search([('company_id', '=', self.company_id.id)])
        #     contract = self.env['collaborators.contract'].search(
        #         [('company_id', '=', self.company_id.id), ('stage', 'in', ('new', 'open', 'done'))])
        #
        #     stt = 1
        #     for hd in contract:
        #         # lấy ra loại hợp đồng
        #         services = hd.crm_line_ids.filtered(lambda rec: rec.state in ('new', 'open'))
        #
        #         for ctv in hd.collaborators_id:
        #             # Lấy ra SO
        #             list_sale_order = SO.filtered(lambda rec: rec.booking_id.collaborators_id.id == ctv.id)
        #
        ...
        #                             ret_data.append({
        #                                 "stt": stt,
        #                                 "ma_ctv": ctv.code_collaborators,
        #                                 "ten_ctv": ctv.collaborators,
    ...
    #     # in du lieu
    #     report_brand_overview_attachment = self.env['ir.attachment'].browse(self.env.ref('crm_collaborators.bao_cao_doanh_thu_hoa_hong_attachment').id)
    ...
    def create_report(self):
        datas = self._get_data_report()
        # in du lieu
        report_brand_overview_attachment = self.env['ir.attachment'].browse(self.env.ref('crm_collaborators.bao_cao_chi_tiet_doanh_so_attachment').id)
        decode = base64.b64decode(report_brand_overview_attachment.datas)
        wb = load_workbook(BytesIO(decode))
        ws = wb.active
        line_font = Font(name='Times New Roman', size=12)
