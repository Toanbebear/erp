...
class DataSet(http.Controller):
    ...
    def call_button(self, model, method, args, kwargs):
        _logger.info('CALLBUTTON: %s | %s | %s | %s' % (model, method, args, kwargs))
...
class ExcelExport(ExportFormat, http.Controller):
    ...
    @serialize_exception
    def index(self, data, token):
        # Trường hợp xuất dữ liệu phiếu khám mà không có công ty tương ứng sẽ bị chặn quyền đọc bác sỹ mặc dù đọc view bình thường.
        # Dòng if bên dưới để trỏ vào trường tên bác sỹ được lưu sẵn thay vì truy cập bác sỹ.
        if '"sh.medical.appointment.register.walkin"' in data and '"doctor"' in data:
            data = data.replace('"doctor"', '"doctor_name"')
        return self.base(data, token)
