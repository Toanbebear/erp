...
class SHealthInpatient(models.Model):
    ...
    def get_domain_department(self):
        ...
        return [('institution','=', institution.id),('type','in',['Surgery','Spa','Laser','Odontology'])]
    ...
    admission_date = fields.Datetime(string='Hospitalization Date', readonly=False, states={'Discharged': [('readonly', True)]})
    ...
    ward = fields.Many2one('sh.medical.health.center.ward', string='Ward', readonly=True, states={'Draft': [('readonly', False)]},default=_get_default_ward, domain=lambda self: self.get_domain_department())
    ...
    services = fields.Many2many('sh.medical.health.center.service', 'sh_inpatient_service_rel', 'inpatient_id', 'service_id',
                               readonly=True, states={'Draft': [('readonly', False)]}, store=True, track_visibility='onchange',
                               string='Dịch vụ')
    is_new_request = fields.Boolean('Chỉ định thêm',states={'Discharged': [('readonly', True)], 'Cancelled': [('readonly', True)]})
