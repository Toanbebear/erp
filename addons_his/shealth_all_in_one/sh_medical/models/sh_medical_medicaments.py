...
class SHealthMedicines(models.Model):
    ...
    def create(self, vals):
        ...
        vals['type'] = 'product'
        #nếu là thuốc thì sẽ tracking by lot - TẮT
        # if vals.get('medicament_type') == 'Medicine':
        #     vals['tracking'] = 'lot'

        if vals.get('medicine_category_id'):
            medicine_category = self.env['sh.medical.medicines.category'].search(
                [('id', '=', vals['medicine_category_id'])])
            sequence = self.env['ir.sequence'].next_by_code(
                'HIS.%s' % (medicine_category.code))
        else:
            raise ValidationError(_('Bạn chưa chọn nhóm cho Thuốc/Vật tư!'))

        #nếu ko nhập default code thì sinh mã theo sequence
        if not vals.get('default_code'):
            if not sequence:
                raise ValidationError(_('Định danh mã Thuốc/Vật tư đang không tồn tại!'))

            vals['default_code'] = sequence

        #tạo thuốc/vật tư
        res = super(SHealthMedicines, self).create(vals)

        #đổi external ID
        ExternalD = self.env['ir.model.data'].sudo()
        prefix = 'THUOC' if res.medicament_type == 'Medicine' else 'VATTU'
        if not ExternalD.search([('model', '=', 'sh.medical.medicines'), ('res_id', '=', res.id)]):
            ExternalD.create(
                {'module': '__import__', 'name':  prefix + '_' + res.default_code, 'model': 'sh.medical.medicines',
                 'res_id': res.id})
            print('Đã tạo ExID cho %s' % (prefix + '_' + res.default_code))

        exID = ExternalD.search([('model', '=', 'product.product'), ('res_id', '=', res.product_id.id)])
        if not exID:
            ExternalD.create(
                {'module': '__import__', 'name':  prefix + '_' + res.default_code + '_product_product', 'model': 'product.product',
                 'res_id': res.product_id.id})
            print('Đã tạo ExID cho %s' % (prefix + '_' + res.default_code + '_product_product'))
