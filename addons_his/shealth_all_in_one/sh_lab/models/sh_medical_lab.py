...
class SHealthLabTestCriteria(models.Model):
    ...
    code_labtest_criteria = fields.Char('Mã thông số')
...
class SHealthLabTests(models.Model):
    ...
    doctor_note = fields.Text(string='Ghi chú', readonly=False,
    ...
    # Áp dụng rule Phiếu Xét nghiệm - share company
    company_id = fields.Many2one('res.company', related='walkin.company_id',
                                 string='Company',
                                 store=True,
                                 readonly=True,
                                 ondelete='cascade')
    company2_id = fields.Many2many('res.company', 'sh_medical_lab_test_company_related_rel', 'lab_test_id', 'company_id',
    ...
    def write(self, vals):
        if vals.get('date_requested') or vals.get('date_analysis') or vals.get('date_done'):
            for record in self:
                ...
                if date_requested and date_analysis and date_done and (
                        date_analysis < date_requested or date_analysis > date_done):
                    raise UserError(
                        'Thông tin không hợp lệ! Ngày phân tích phải sau ngày yêu cầu và trước ngày trả kết quả!')
    ...
    def create(self, vals):
        ...
        if not sequence:
            raise ValidationError('Định danh phiếu Xét nghiệm của Cơ sở y tế này đang không tồn tại!')

        vals['name'] = sequence or '/'
        vals['lab_test_criteria'] = False
        ...
        if not res.lab_test_criteria:
            ...
            if data_case:
                has_child = True
            res.write({'lab_test_criteria': data_case, 'has_child': has_child})
    ...
    def onchange_test_type_id(self):
        ...
        self.lab_test_criteria = [(0, 0, {'name': c.name, 'sequence': c.sequence, 'normal_range': c.normal_range,
                                          'abnormal': c.abnormal, 'units': c.units, 'lab_test_criteria_id': c.id})
                                  for c in self.test_type.lab_criteria] if self.test_type.lab_criteria else False
        self.lab_test_material_ids = [(0, 0, {'sequence': m.sequence, 'product_id': m.product_id,
                                              'init_quantity': m.init_quantity, 'quantity': m.quantity, 'is_init': True,
                                              'uom_id': m.uom_id})
    ...
    def onchange_test_type_id_values(self, test_type):
        ...
        query = _(
            "select name, sequence, normal_range, abnormal, units, id from sh_medical_labtest_criteria where medical_type_id=%s") % (
                    str(test_type))
    ...
    def set_to_test_inprogress(self):
        if self.state == 'Completed':
            ...
            self.walkin.update_walkin_material(mats_types=['Labtest'])
        ...
        Walkin.write(
            {'lab_test_done_count': lt_done})  # , 'has_lab_result': True if lt_total - lt_done == 0 else False})
        # return res
    ...
    def reverse_materials(self):
        ...
        pick_need_reverse = self.env['stock.picking'].search(
            [('origin', 'ilike', key_words), ('company_id', '=', self.env.company.id)],
    ...
    def set_to_test_complete(self):
        ...
        if vals and validate_str == '':
            ...
            stock_picking.with_context(
                force_period_date=date_done).sudo().button_validate()  # ham tru product trong inventory, sudo để đọc stock.valuation.layer
            ...
            self.lab_test_material_ids.write({'picking_id': stock_picking.id})
...
class SHealthLabTestMaterial(models.Model):
    ...
    services = fields.Many2many('sh.medical.health.center.service', 'sh_lab_test_material_service_rel',
                                track_visibility='onchange', string='Dịch vụ thực hiện')
    ...
    @api.depends('quantity', 'init_quantity')
    def compute_qty_used_bom(self):
        for record in self:
            if record.quantity > record.init_quantity:
        ...
        for record in self:
            ...
            if record.product_id:
                quantity_on_hand = self.env['stock.quant'].with_user(1)._get_available_quantity(
                    record.product_id.product_id, record.location_id)  # check quantity trong location
                qty_in_loc = record.uom_id._compute_quantity(quantity_on_hand,
                                                             record.product_id.uom_id) if record.uom_id != record.product_id.uom_id else quantity_on_hand
    ...
    def _change_product_id(self):
        ...
        institution = self.lab_test_id.institution or self.env['sh.medical.health.center'].search(
            [('his_company', '=', self.env.companies.ids[0])], limit=1)
        room = self.lab_test_id.perform_room or self.env['sh.medical.health.center.ot'].browse(
            self.env.context.get('room'))
        self.uom_id = self.product_id.uom_id
        if self.product_id and room and institution:
            if self.medicament_type == 'Medicine':
                ...
                domain['domain']['location_id'] = [('location_institution_type', '=', 'medicine'),
                                                   ('company_id', '=', institution.his_company.id)]
...
class SHealthLabTestsResultCriteria(models.Model):
    ...
    lab_test_criteria_id = fields.Many2one('sh.medical.labtest.criteria', string='Lab Test Criteria ID')
