...
ListRenderer.include({
                ...
                if (group.groupedBy.length) {
                    ...
                    var $records = _.map(group.data, function (record,index) {
                    	...
                    	if (_self.mode !== 'edit' || _self.hasSelectors){
                    		return self._renderRow(record).prepend($("<th class='o_list_row_count_sheliya'>").html(index+1+group.offset)); //.prepend($('<td>'));
                    	}
                    	else{
    ...
    _renderRow: function (record) {
    	...
    	if (this.mode !== 'edit' && this.state.groupedBy.length==0){
	    	...
	    	if (index!==-1){
	    		$row.prepend($("<th class='o_list_row_count_sheliya'>").html(index+1+this.state.offset));
	    	}
    	}
    	return $row;
