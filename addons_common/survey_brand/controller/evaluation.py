...
class SurveyBrandEvaluation(Survey):

    @http.route('/survey_brand/evaluation/<int:evaluation_id>', type='http', auth='public', website=True)
    def survey_brand_evaluation_init(self, evaluation_id, **post):
        ...
        """
        evaluation = request.env['sh.medical.evaluation'].sudo().browse(evaluation_id)
        partner = evaluation.patient.partner_id
        customer_phone = partner.phone if partner.phone else partner.mobile
        timing = request.env['survey.survey.type'].sudo().search([('is_tk', '=', True)])
        ...
        data = {
            ...
            'branch': evaluation.institution.his_company.name,
            ...
            'booking': booking.id,
            'timing': timing,
            'data_submit': '/survey_brand/evaluation/%s/survey' % evaluation_id,
        }
    ...
    @http.route('/survey_brand/evaluation/<int:evaluation_id>/survey', type='http', auth='public', website=True)
    def get_evaluation_surveys(self, evaluation_id, **post):
        evaluation = request.env['sh.medical.evaluation'].sudo().browse(evaluation_id)
        partner = evaluation.patient.partner_id
        customer_phone = partner.phone if partner.phone else partner.mobile
        ...
        if post and 'survey_survey' in post and 'group_service' in post and 'time' in post:
            ret = {'redirect': '/survey_brand/evaluation/%s/%s?group_service=%s&time=%s' % (evaluation_id, post['survey_survey'],post['group_service'], post['time'])}
    ...
    @http.route('/survey_brand/evaluation/<string:evaluation_id>/<string:survey_token>', type='http', auth='public',
                website=True)
    def survey_brand_evaluation_start(self, evaluation_id, survey_token, **post):
        """
        """
        answer_token = None
        ...
        if not answer_sudo:
            try:
                ...
                answer_sudo = survey_sudo._create_answer_evaluation(user=request.env.user, evaluation_id=evaluation_id, group_service_id=group_service, survey_time_id=survey_time_id )
            except UserError:
                answer_sudo = False
        if not answer_sudo:
            try:
        ...
        return request.redirect(
            '/survey_brand/fill/evaluation/%s/%s/%s' % (evaluation_id, survey_sudo.access_token, answer_sudo.token))
        # eva = request.env['sh.medical.evaluation'].sudo().browse(int(evaluation_id))
        # time_id = None
        ...
        #     'partner_id': eva.walkin.booking_id.partner_id.id,
        #     'state': 'new',
        #     'crm_id': eva.walkin.booking_id.id,
        #     'walkin_id': eva.walkin.id,
    ...
    @http.route('/survey_brand/evaluation/<string:evaluation_id>/<string:survey_token>', type='http', auth='public',
                website=True)
    def survey_brand_start_evaluation(self, evaluation_id, survey_token, answer_token=None, email=False, **post):
    ...
    @http.route('/survey_brand/fill/evaluation/<int:evaluation_id>/<string:survey_token>/<string:answer_token>',
                type='http',
                auth='public',
                website=True)
    def survey_brand_evaluation_display_page_base(self, evaluation_id, survey_token, answer_token, prev=None, **post):
        access_data = self._get_access_data(survey_token, answer_token, ensure_token=True)
        if access_data['validity_code'] is not True:
            return self._redirect_with_error_survey_brand(access_data, access_data['validity_code'])
        evaluation = request.env['sh.medical.evaluation'].sudo().browse(int(evaluation_id))
        walkin = request.env['sh.medical.appointment.register.walkin'].sudo().browse(int(evaluation.walkin.id))
