...
GENDER = [('male', 'Male'), ('female', 'Female'), ('transguy', 'Transguy'), ('transgirl', 'Transgirl'), ('other', 'Other')]
...
TEMP_LINE = dict((key, None) for key in TEMPLATE)

class InsuranceServicePerformanceReportKangnam(models.TransientModel):
    _name = 'insurance.service.perform.report.kangnam'
    ...
    def _get_data_report(self):
        ...
        Surgery = self.env['sh.medical.surgery']
        Specialty = self.env['sh.medical.specialty']
        ...
        if self.type == 'All':
            ...
            for surgery in data_surgery:
                ...
                for ser_perform in surgery.services:
                    sol = so.order_line.filtered(lambda r: r.product_id == ser_perform.product_id and r.price_subtotal != 0)
                    money = sum([rec_sol.price_subtotal for rec_sol in sol])
                    buoi_thu = self.walkin_num(Walkin, ser_perform)
                    consultants = list()
                    ...
                    for index in range(1, len(consultants) + 1):
                        ...
                        ret_data.append({
                            'ma_nv_tu_van': consultants[index - 1][0].employee_code if len(consultants) != 0 else None,
                            'ten_nv_tu_van': consultants[index - 1][0].name if len(consultants) != 0 else None,
                            'bo_phan_nv_tu_van': consultants[index - 1][0].group_job.name if len(consultants) != 0 else None,
                            'chuc_danh_nv_tu_van': consultants[index - 1][0].job_id.name if len(consultants) != 0 else None,
                            'vai_tro_nv_tu_van': consultants[index - 1][1] if len(consultants) != 0 else None,
                            ...
                            'ngay_sinh': so.partner_id.birth_date.strftime('%d/%m/%Y') if so.partner_id.birth_date else None,
                            'dia_chi': so.partner_id.street,
                            'ma_dich_vu': ser_perform.default_code,
                    ...
                    total_revenue += money
            # Sap - Laser - Odontology
            domain_specialty.append(('department_type', 'in', ('Spa', 'Laser', 'Odontology')))
            ...
            for specialty in data_specialty:
                ...
                for spe_perform in specialty.services:
                    buoi_thu = self.walkin_num(Walkin, spe_perform)
                    # get s ale order line for service
                    sol = so.order_line.filtered(lambda r: r.product_id == spe_perform.product_id)
                    money = sum([rec_sol.price_subtotal for rec_sol in sol])
                    consultants = list()
                    if sol.crm_line_id.consultants_1:
                        consultants.append((sol.crm_line_id.consultants_1.employee_ids[0], sol.crm_line_id.consulting_role_1))
                    ...
                    for index in range(1, len(consultants) + 1):
                        ret_data.append({
                            ...
                            'ngay_sinh': so.partner_id.birth_date.strftime('%d/%m/%Y') if so.partner_id.birth_date else None,
                            'dia_chi': so.partner_id.street,
                            'ma_dich_vu': spe_perform.default_code,
                            'thu_tu_tu_van': 'Tư vấn viên %s' % str(index) if len(consultants) != 0 else None,
                            'ma_nv_nguoi_tao': so.booking_id.create_by.employee_ids[0].employee_code if so.booking_id.create_by.employee_ids else None,
                            'nguoi_tao': so.booking_id.create_by.name,
                            'phong_ban_nguoi_tao': so.booking_id.create_by.employee_ids[0].department_id.name if so.booking_id.create_by.employee_ids else None
                    ...
                    total_revenue += money
            temp_line = TEMP_LINE.copy()
            temp_line['doanh_so'] = total_revenue
            ret_data.append(temp_line)
                ...
                for spe_perform in specialty.services:
                    ...
                    for index in range(1, len(consultants) + 1):
                        ret_data.append({
                            ...
                            'ma_khach_hang': so.code_customer,
                            'gioi_tinh': DICT_GENDER.get(so.partner_id.gender),
                            'ngay_sinh': so.partner_id.birth_date.strftime('%d/%m/%Y'),
                            'dia_chi': so.partner_id.street,
                            'ma_dich_vu': spe_perform.default_code,
                            'thu_tu_tu_van': 'Tư vấn viên %s' % str(index) if len(consultants) != 0 else None,
                            'ma_nv_nguoi_tao': so.booking_id.create_by.employee_ids[0].employee_code if so.booking_id.create_by.employee_ids else None,
    ...
    def create_report_service_perform_report(self):
        ...
        report_brand_overview_attachment = self.env['ir.attachment'].browse(
            self.env.ref('report_sale.bao_cao_doanh_thu_thuc_hien_dich_vu_bao_hanh_kangnam_attachment').id)
