...
class ModelName(models.TransientModel):
    ...
    def report_daily_sales(self):
        ...
        SELECT 
            ...
            pr.name as user,
            '' as note,
            rpr.name as create_user,
            lead.create_by_department as create_by_department
        FROM account_payment as payment
            ...
            LEFT JOIN res_users as c_user ON c_user.id = lead.create_by
            LEFT JOIN res_partner as rpr on rpr.id = c_user.partner_id
        WHERE 
        ...
        if len(data_select) > 0:
            ...
            daily_sales_attachment = self.env['ir.attachment'].browse(
                self.env.ref('report_sale.report_sale_erp_daily_attachment').id)
            ...
            style = Font(name='Times New Roman', color='00000000', bold=False, underline='none', size=14)
