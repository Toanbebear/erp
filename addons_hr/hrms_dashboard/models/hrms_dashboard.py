...
class Employee(models.Model):
    ...
    def get_user_employee_details(self):
        ...
        emp = self.env['hr.employee'].sudo().search([('user_id', '=', uid)], limit=1)
        allocation_used_display = emp.allocation_used_display
        allocation_display = emp.allocation_display
        leaves_to_approve = self.env['hr.leave'].sudo().search_count([('state', 'in', ['confirm', 'validate1']), ('employee_id.user_id', '=', uid)])
        ...
        leaves_this_month = 0
        leaves = self.env['hr.leave'].sudo().search([('employee_id.user_id', '=', uid)])
        for ret in leaves:
            if ret.date_from.month == month or ret.date_to.month == month:
                leaves_this_month += 1
            else:
                continue
        ...
        # timesheet_count = self.env['hr.attendance'].sudo().search_count(
        #     [('employee_id.user_id', '=', uid)])
        timesheet_count = 0
        attendance = self.env['hr.attendance'].sudo().search(
            [('employee_id.user_id', '=', uid)])
        for rec in attendance:
            if rec.check_in and rec.check_in.month == month:
                timesheet_count += 1
            elif rec.check_out and rec.check_out.month == month:
                timesheet_count += 1
            else:
                continue
        ...
        if employee:
            ...
            if employee:
                data = {
                    ...
                    'allocation_used_display': allocation_used_display
                }
